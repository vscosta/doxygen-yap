...
'$meta_expansion_loop'(I,D,G,NG,HVars,CurMod,M,HM) :- 
	...
	arg(I,G,A), 
	'$to_meta'(A, CurMod, HVars, IA, NMod),
	!,
	( var(A) -> NA = IA ;
	  '$meta_expansion'( IA, CurMod, M, HM, NA, HVars)
	),
	arg(I, NG, NA),
	I1 is I-1,
...
'$to_meta'(V, M, HVars, V, M) :- 
    var(V), !, '$not_in_vars'(V,HVars).
'$to_meta'(G, M, _, G1, Mod) :-
    strip_module(M:G, Mod, G1),
    nonvar( G1 ),
    G1 \= _:_,
    nonvar( Mod ),
    functor(G1,F,N),
...
%	'$pred_goal_expansion_on',
	expand_goal(CurMod:G, GI),
	\+ '$same_call'( CurMod:G, GI ), !,
...
expand_goal(M:G, NG) :-
        strip_module( G, M, GI),
	'$do_expand'(GI, M, GO),
	strip_module(M:GO, M1, GOO),
	( M == prolog, '$system_predicate'(GOO, M1) -> 
	      NG = GOO
		       ;
	      NG = M1:GOO
	).
...
% meta-call.
'$match_mod'(G, HMod, _, HM, NG) :- 
    strip_module(HMod:G, HM0, NG), HM0 == HM, !.
'$match_mod'(G, GMod, _, _,  NMod:GN) :-
    strip_module(GMod:G, NMod, GN).
...
'$module_u_vars'(I,D,H,[Y|L]) :-
	...
	arg(I,H,HI), ( var(HI) -> Y = HI ; Y = same( HI, _) ), !,
	I1 is I-1,
