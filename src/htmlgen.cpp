...
// this
// part will be rendered inside menu.js
static void writeServerSearchBox(FTextStream &t, const char *relPath,
  ...
  t << "              <img id=\"MSearchSelect\" src=\"" << relPath
    << "search/mag.png\" alt=\"\"/>\n";
  if (!highlightSearch) {
    t << "              <input type=\"text\" id=\"MSearchField\" "
         "name=\"query\" value=\""
...
QCString clearBlock(const char *s, const char *begin, const char *end) {
  ...
  char *r;
  for (r = result.rawData(), p = s; (q = strstr(p, begin)) != 0;
       p = q + endLen) {
    int l = (int)(q - p);
    memcpy(r, p, l);
    r += l;
...
static QCString getSearchBox(bool serverSide, QCString relPath,
                             bool highlightSearch) {
  QGString result;
...
static QCString substituteHtmlKeywords(const QCString &s, const QCString &title,
  ...
  static bool titleArea = (hasProjectName || hasProjectBrief ||
                           hasProjectLogo || (disableIndex && searchEngine));
  ...
  if (timeStamp) {
    generatedBy = theTranslator->trGeneratedAt(
        dateToString(TRUE), convertToHtml(Config_getString(PROJECT_NAME)));
  } else {
    generatedBy = theTranslator->trGeneratedBy();
  }

  if (treeView) {
    treeViewCssJs = "<link href=\"$relpath^navtree.css\" rel=\"stylesheet\" "
                    "type=\"text/css\"/>\n"
                    "<script type=\"text/javascript\" "
                    "src=\"$relpath^resize.js\"></script>\n"
                    "<script type=\"text/javascript\" "
                    "src=\"$relpath^navtreedata.js\"></script>\n"
                    "<script type=\"text/javascript\" "
                    "src=\"$relpath^navtree.js\"></script>\n"
                    "<script type=\"text/javascript\">\n"
  ...
  if (searchEngine) {
    ...
    if (!serverBasedSearch) {
      searchCssJs += "<script type=\"text/javascript\" "
                     "src=\"$relpath^search/searchdata.js\"></script>\n";
    }
    searchCssJs += "<script type=\"text/javascript\" "
  ...
  if (mathJax) {
    ...
    mathJaxJs += "</script>";
    mathJaxJs += "<script type=\"text/javascript\" src=\"" + path +
                 "MathJax.js\"></script>\n";
  }

  // first substitute generic keywords
  QCString result = substituteKeywords(
      s, title, convertToHtml(Config_getString(PROJECT_NAME)),
  ...
  result = substitute(result, "$relpath$",
                      relPath); //<-- obsolete: for backwards compatibility only
...
HtmlCodeGenerator::HtmlCodeGenerator() : m_streamSet(FALSE), m_col(0) {}
...
void HtmlCodeGenerator::_writeCodeLink(const char *className, const char *ref,
                                       const char *f, const char *anchor,
...
void HtmlCodeGenerator::writeTooltip(const char *id, const DocLinkInfo &docInfo,
  ...
  if (desc) {
    m_t << "<div class=\"ttdoc\">";
    docify(desc); // desc is already HTML escaped; but there are still < and >
                  // signs
    m_t << "</div>";
  }
      ...
      if (Config_getBool(SOURCE_BROWSER) && Config_getBool(SOURCE_TOOLTIPS)) {
        t << endl
          << "$(document).ready(function() {\n"
             ...
             "    $(this).powerTip({ placement: 's', smartPlacement: true, "
             "mouseOnToPopup: true });\n"
             "  });\n"
             "});\n";
      }
...
void HtmlGenerator::writeSearchData(const char *dir) {
  static bool serverBasedSearch = Config_getBool(SERVER_BASED_SEARCH);
  // writeImgData(dir,serverBasedSearch ? search_server_data :
  // search_client_data);
  ResourceMgr &mgr = ResourceMgr::instance();
...
void HtmlGenerator::writeStyleSheetFile(QFile &file) {
  FTextStream t(&file);
  t << replaceColorMarkers(
      substitute(ResourceMgr::instance().getAsString("doxygen.css"),
...
void HtmlGenerator::startFile(const char *name, const char *,
  ...
  if (fileName.right(Doxygen::htmlFileExtension.length()) !=
  ...
  lastFile = fileName;
  t << substituteHtmlKeywords(g_header, convertToHtml(filterTitle(title)),
                              relPath);

  t << "<!-- " << theTranslator->trGeneratedBy() << " Doxygen " << versionString
    << " -->" << endl;
  // static bool generateTreeView = Config_getBool(GENERATE_TREEVIEW);
  static bool searchEngine = Config_getBool(SEARCHENGINE);
  if (searchEngine /*&& !generateTreeView*/) {
    t << "<script type=\"text/javascript\">\n";
    t << "var searchBox = new SearchBox(\"searchBox\", \"" << relPath
...
void HtmlGenerator::writeSearchInfo() { writeSearchInfo(t, relPath); }

QCString HtmlGenerator::writeLogoAsString(const char *path) {
  static bool timeStamp = Config_getBool(HTML_TIMESTAMP);
  QCString result;
  if (timeStamp) {
    result += theTranslator->trGeneratedAt(dateToString(TRUE),
...
void HtmlGenerator::writeLogo() { t << writeLogoAsString(relPath); }
...
void HtmlGenerator::endFile() { endPlainFile(); }

void HtmlGenerator::startProjectNumber() { t << "<h3 class=\"version\">"; }

void HtmlGenerator::endProjectNumber() { t << "</h3>"; }

void HtmlGenerator::writeStyleInfo(int part) {
  // printf("writeStyleInfo(%d)\n",part);
  if (part == 0) {
    if (Config_getString(HTML_STYLESHEET)
            .isEmpty()) // write default style sheet
    {
      ...
      // endl;
      // t << "     border-style : double; border-color : blue; padding-left :
      // 1em; padding-right : 1em }" << endl;
...
void HtmlGenerator::writeString(const char *text) { t << text; }

void HtmlGenerator::startIndexListItem() { t << "<li>"; }

void HtmlGenerator::endIndexListItem() { t << "</li>" << endl; }
...
void HtmlGenerator::endTextLink() { t << "</a>"; }
...
void HtmlGenerator::endHtmlLink() { t << "</a>"; }
...
void HtmlGenerator::startSection(const char *lab, const char *,
...
void HtmlGenerator::docify(const char *str) { docify(str, FALSE); }
...
static void startSectionHeader(FTextStream &t, const QCString &relPath,
                               int sectionCount) {
...
void HtmlGenerator::endClassDiagram(const ClassDiagram &d, const char *fileName,
                                    const char *name) {
  endSectionHeader(t);
...
void HtmlGenerator::startIndexList() { t << "<table>" << endl; }

void HtmlGenerator::endIndexList() { t << "</table>" << endl; }
...
void HtmlGenerator::endIndexKey() { t << "</td>"; }
...
void HtmlGenerator::endDotGraph(const DotClassGraph &g) {
  ...
  g.writeGraph(t, GOF_BITMAP, EOF_Html, dir, fileName, relPath, TRUE, TRUE,
               m_sectionCount);
...
void HtmlGenerator::endInclDepGraph(const DotInclDepGraph &g) {
  ...
  g.writeGraph(t, GOF_BITMAP, EOF_Html, dir, fileName, relPath, TRUE,
               m_sectionCount);
...
void HtmlGenerator::endMemberGroupHeader() { t << "</div></td></tr>" << endl; }

void HtmlGenerator::startMemberGroupDocs() {
  t << "<tr><td colspan=\"2\"><div class=\"groupText\">";
}

void HtmlGenerator::endMemberGroupDocs() { t << "</div></td></tr>" << endl; }
...
void HtmlGenerator::startSimpleSect(SectionTypes, const char *filename,
                                    const char *anchor, const char *title) {
  t << "<dl><dt><b>";
  if (filename) {
...
void HtmlGenerator::endSimpleSect() { t << "</dl>"; }
...
void HtmlGenerator::endParamList() { t << "</dl>"; }
...
static void startQuickIndexList(FTextStream &t, bool compact,
                                bool topLevel = TRUE) {
  if (compact) {
    if (topLevel) {
...
static void startQuickIndexItem(FTextStream &t, const char *l, bool hl,
                                bool /*compact*/, const QCString &relPath) {
  t << "      <li";
  if (hl) {
    t << " class=\"current\"";
  }
...
static void renderQuickLinksAsTree(FTextStream &t, const QCString &relPath,
                                   LayoutNavEntry *root)
...
static void renderQuickLinksAsTabs(FTextStream &t, const QCString &relPath,
  ...
  {
    renderQuickLinksAsTabs(t, relPath, hlEntry->parent(), kind, highlightParent,
                           highlightSearch);
  }
  if (hlEntry->parent() &&
      hlEntry->parent()->children().count() >
          0) // draw tabs for row containing hlEntry
  {
    ...
    if (count > 0) // at least one item is visible
    {
      ...
      if (hlEntry->parent() ==
          LayoutDocManager::instance().rootNavEntry()) // first row is special
                                                       // as it contains the
                                                       // search box
      {
        static bool searchEngine = Config_getBool(SEARCHENGINE);
        ...
        if (!highlightSearch) // on the search page the index will be ended by
                              // the
        // page itself
        {
...
static void writeDefaultQuickLinks(FTextStream &t, bool compact,
                                   HighlightedItem hli, const char *file,
                                   const QCString &relPath) {
  static bool serverBasedSearch = Config_getBool(SERVER_BASED_SEARCH);
  static bool searchEngine = Config_getBool(SEARCHENGINE);
  static bool externalSearch = Config_getBool(EXTERNAL_SEARCH);
  ...
  if (compact) {
    ...
    t << "<script type=\"text/javascript\" src=\"" << relPath
      << "menudata.js\"></script>" << endl;
    t << "<script type=\"text/javascript\" src=\"" << relPath
      << "menu.js\"></script>" << endl;
    t << "<script type=\"text/javascript\">" << endl;
    t << "$(function() {" << endl;
    t << "  initMenu('" << relPath << "'," << (searchEngine ? "true" : "false")
      << "," << (serverBasedSearch ? "true" : "false") << ",'" << searchPage
...
void HtmlGenerator::endQuickIndices() { t << "</div><!-- top -->" << endl; }
...
QCString HtmlGenerator::writeSplitBarAsString(const char *name,
  ...
  if (generateTreeView) {
    result =
        ...
        QCString(name) + Doxygen::htmlFileExtension + QCString("','") +
        relpath + QCString("');});\n"
                           "</script>\n"
                           "<div id=\"doc-content\">\n");
  }
...
void HtmlGenerator::startContents() { t << "<div class=\"contents\">" << endl; }

void HtmlGenerator::endContents() { t << "</div><!-- contents -->" << endl; }
...
void HtmlGenerator::writeSearchPage() {
  ...
  if (cf.open(IO_WriteOnly)) {
    ...
    t << "    2 => \""
      << substitute(theTranslator->trSearchResults(2), "$", "\\$") << "\",\n";
    t << "  ),\n";
    ...
    t << "  'split_bar' => \""
      << substitute(
             substitute(writeSplitBarAsString("search", ""), "\"", "\\\""),
             "\n", "\\n")
      << "\",\n";
    t << "  'logo' => \""
      << substitute(substitute(writeLogoAsString(""), "\"", "\\\""), "\n",
                    "\\n")
      << "\",\n";
    t << ");\n\n";
    t << "</script>\n";
...
void HtmlGenerator::writeExternalSearchPage() {
  ...
  static bool disableIndex = Config_getBool(DISABLE_INDEX);
  QCString fileName =
      Config_getString(HTML_OUTPUT) + "/search" + Doxygen::htmlFileExtension;
  QFile f(fileName);
  if (f.open(IO_WriteOnly)) {
    FTextStream t(&f);
    t << substituteHtmlKeywords(g_header, "Search", "");
    ...
    if (!Config_getBool(DISABLE_INDEX)) {
      writeDefaultQuickLinks(t, TRUE, HLI_Search, 0, "");
      t << "            <input type=\"text\" id=\"MSearchField\" "
           "name=\"query\" value=\"\" size=\"20\" accesskey=\"S\" "
    ...
    t << "    <div class=\"title\">" << theTranslator->trSearchResultsTitle()
      << "</div>" << endl;
    t << "  </div>" << endl;
    t << "</div>" << endl;
  ...
  if (sf.open(IO_WriteOnly)) {
    ...
    while (ml) {
      ...
      if (eqPos != -1) // tag command contains a destination
      {
        ...
        QCString destName =
            mapLine.right(mapLine.length() - eqPos - 1).stripWhiteSpace();
        if (!tagName.isEmpty()) {
          if (!first)
            t << "," << endl;
          t << "  \"" << tagName << "\": \"" << destName << "\"";
    ...
    t << "    results.html('<p>" << theTranslator->trSearchResults(0)
      << "</p>');" << endl;
    t << "  }" << endl;
    t << "});" << endl;
  } else {
...
void HtmlGenerator::endConstraintParam() { t << "</em></td>"; }

void HtmlGenerator::startConstraintType() {
  t << "<td>&#160;:</td><td valign=\"top\"><em>";
}

void HtmlGenerator::endConstraintType() { t << "</em></td>"; }

void HtmlGenerator::startConstraintDocs() { t << "<td>&#160;"; }

void HtmlGenerator::endConstraintDocs() { t << "</td></tr>" << endl; }
...
void HtmlGenerator::endHeaderSection() { t << "</div><!--header-->" << endl; }
...
void HtmlGenerator::writeInheritedSectionTitle(const char *id, const char *ref,
                                               const char *file,
                                               const char *anchor,
                                               const char *title,
                                               const char *name) {
...
void HtmlGenerator::writeSummaryLink(const char *file, const char *anchor,
                                     const char *title, bool first) {
  if (first) {
    t << "  <div class=\"summary\">\n";
  } else {
    t << " &#124;\n";
...
void HtmlGenerator::setCurrentDoc(Definition *context, const char *anchor,
                                  bool isSourceFile) {
