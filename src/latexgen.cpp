...
LatexCodeGenerator::LatexCodeGenerator(FTextStream &t, const QCString &relPath,
...
#define COPYCHAR()                                                             \
  do {                                                                         \
    ...
    m_col++;                                                                   \
        ...
        {
          while (m_col < maxLineLen && (c = *p) && c != 0x0c && c != '\t' &&
                 c != '\n' && c != ' ') {
            COPYCHAR();
          }
          if (m_col >= maxLineLen) // force line break
...
void LatexCodeGenerator::writeLineNumber(const char *ref, const char *fileName,
...
void LatexCodeGenerator::startCodeLine(bool) { m_col = 0; }

void LatexCodeGenerator::endCodeLine() { codify("\n"); }

void LatexCodeGenerator::startFontClass(const char *name) {
  m_t << "\\textcolor{" << name << "}{";
}

void LatexCodeGenerator::endFontClass() { m_t << "}"; }
...
static void writeLatexMakefile() {
  ...
  {
    t << "all: refman.pdf" << endl << endl << "pdf: refman.pdf" << endl << endl;
...
static void writeMakeBat() {
  ...
  t << "set Dir_Old=%cd%\n";
  t << "cd /D %~dp0\n\n";
  t << "del /s /f *.ps *.dvi *.aux *.toc *.idx *.ind *.ilg *.log *.out *.brf "
       "*.blg *.bbl refman.pdf\n\n";
  if (!Config_getBool(USE_PDFLATEX)) // use plain old latex
  {
    ...
    t << ":repeat\n";
    t << "set content=X\n";
    t << "for /F \"tokens=*\" %%T in ( 'findstr /C:\"Rerun LaTeX\" refman.log' "
         ") do set content=\"%%~T\"\n";
    t << "if !content! == X for /F \"tokens=*\" %%T in ( 'findstr /C:\"Rerun "
         "to get cross-references right\" refman.log' ) do set "
         "content=\"%%~T\"\n";
    ...
    if (!fileName.isEmpty()) {
      ...
      if (fi.exists()) {
        if (checkExtension(fi.fileName().data(), latexStyleExtension)) {
          // strip the extension, it will be added by the usepackage in the tex
          // conversion process
          t << "\\usepackage{"
            << stripExtensionGeneral(fi.fileName().data(), latexStyleExtension)
            << "}\n";
        } else {
...
static void writeDefaultFooter(FTextStream &t) {
  ...
  t << "\\newpage\n"
       ...
       "\\addcontentsline{toc}{"
    << unit << "}{" << theTranslator->trRTFGeneralIndex()
    << "}\n"
       "\\printindex\n"
       "\n"
       "\\end{document}\n";
...
void LatexGenerator::startProjectNumber() { t << "\\\\[1ex]\\large "; }
...
void LatexGenerator::writeString(const char *text) { t << text; }
...
void LatexGenerator::endHtmlLink() { t << "}"; }
...
void LatexGenerator::startIndexKey() { t << "\\item\\contentsline{section}{"; }
...
void LatexGenerator::startPageRef() { t << " \\doxyref{}{"; }
...
void LatexGenerator::startMemberDoc(const char *clname, const char *memname,
  ...
  static const char *levelLab[] = {"subsubsection", "paragraph", "subparagraph",
                                   "subparagraph"};
  static bool compactLatex = Config_getBool(COMPACT_LATEX);
  static bool pdfHyperlinks = Config_getBool(PDF_HYPERLINKS);
  int level = 0;
  ...
  if (memTotal > 1) {
    t << "\\hspace{0.1cm}{\\footnotesize\\ttfamily [" << memCount << "/"
      << memTotal << "]}";
  }
  t << "}";
...
void LatexGenerator::startSection(const char *lab, const char *,
...
void LatexGenerator::startClassDiagram() {
  // if (Config_getBool(COMPACT_LATEX)) t << "\\subsubsection"; else t <<
  // "\\subsection";
  // t << "{";
}
...
void LatexGenerator::startMemberGroupDocs() { t << "{\\em "; }

void LatexGenerator::endMemberGroupDocs() { t << "}"; }
...
void LatexGenerator::startDotGraph() { newParagraph(); }

void LatexGenerator::endDotGraph(const DotClassGraph &g) {
  g.writeGraph(t, GOF_EPS, EOF_LaTeX, Config_getString(LATEX_OUTPUT), fileName,
               relPath);
}
...
void LatexGenerator::startDescription() { t << "\\begin{description}" << endl; }
...
void LatexGenerator::endSimpleSect() { t << "\\end{Desc}" << endl; }
...
void LatexGenerator::endParamList() { t << "\\end{Desc}" << endl; }
...
void LatexGenerator::endParameterType() { t << "}]"; }

void LatexGenerator::startParameterName(bool /*oneArgOnly*/) { t << "{"; }

void LatexGenerator::endParameterName(bool last, bool /*emptyList*/,
                                      bool closeBracket) {
  t << " }";
  if (last) {
...
void LatexGenerator::writeDoc(DocNode *n, Definition *ctx, MemberDef *) {
  LatexDocVisitor *visitor = new LatexDocVisitor(
      t, *this, ctx ? ctx->getDefFileExtension() : QCString(""), insideTabbing);
  n->accept(visitor);
  delete visitor;
}
...
void LatexGenerator::startConstraintParam() { t << "\\item[{\\em "; }

void LatexGenerator::endConstraintParam() {}

void LatexGenerator::startConstraintType() { t << "} : {\\em "; }

void LatexGenerator::endConstraintType() { t << "}]"; }
...
void LatexGenerator::startCodeFragment() { t << "\n\\begin{DoxyCode}\n"; }
...
void LatexGenerator::endInlineHeader() { t << "}" << endl; }
...
void LatexGenerator::endInlineMemberDoc() { t << "\\\\\n\\hline\n" << endl; }

void LatexGenerator::startLabels() { t << "\\hspace{0.3cm}"; }
