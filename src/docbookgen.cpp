...
static void writeDocbookHeaderMainpage(FTextStream &t, QCString &pageName) {
  ...
  ;
  t << "<chapter xmlns=\"http://docbook.org/ns/docbook\" version=\"5.0\" "
...
static void writeDocbookHeader_ID(FTextStream &t, QCString id) {
  t << "<?xml version='1.0' encoding='UTF-8' standalone='no'?>" << endl;
  ;
  t << "<section xmlns=\"http://docbook.org/ns/docbook\" version=\"5.0\" "
       "xmlns:xlink=\"http://www.w3.org/1999/xlink\" xml:id=\"_"
    << id << "\">" << endl;
}
...
void writeDocbookLink(FTextStream &t, const char * /*extRef*/,
                      const char *compoundId, const char *anchorId,
...
DocbookCodeGenerator::DocbookCodeGenerator(FTextStream &t)
    : m_lineNumber(-1), m_col(0), m_insideCodeLine(FALSE),
...
DocbookCodeGenerator::DocbookCodeGenerator()
    : m_lineNumber(-1), m_col(0), m_insideCodeLine(FALSE),
      m_insideSpecialHL(FALSE), m_streamSet(FALSE) {
  m_prettyCode = Config_getBool(DOCBOOK_PROGRAMLISTING);
}
...
void DocbookCodeGenerator::writeTooltip(const char *, const DocLinkInfo &,
...
static void writeTemplateArgumentList(ArgumentList *al, FTextStream &t,
  ...
  if (al) {
    ...
    for (ali.toFirst(); (a = ali.current()); ++ali) {
      ...
      if (!a->defval.isEmpty()) {
        t << indentStr << "    <defval>";
        linkifyText(TextGeneratorDocbookImpl(t), scope, fileScope, 0,
                    a->defval);
        t << "</defval>" << endl;
      }
      t << indentStr << "  </param>" << endl;
...
static void writeDocbookDocBlock(FTextStream &t, const QCString &fileName,
...
static void generateTOC(FTextStream &t, PageDef *pd) {
  if (pd->localToc().isDocbookEnabled()) {
    t << "    <toc>" << endl;
    t << "    <title>" << theTranslator->trRTFTableOfContents() << "</title>"
      << endl;
    ...
    for (li.toFirst(); (si = li.current()); ++li) {
      if (si->type == SectionInfo::Section ||
        ...
        if (nextLevel <= maxLevel) {
          ...
          t << "      <tocentry>"
            << (si->title.isEmpty() ? si->label : titleDoc) << "</tocentry>"
            << endl;
        }
        inLi[nextLevel] = TRUE;
        level = nextLevel;
...
static void generateSourceRefList(FTextStream &t, const char *scopeName,
...
static void generateInlineCode(FTextStream &t, const char *scopeName,
                               Definition *def) {
  ...
  if (inlineSources && def->hasSources()) {
    ...
    int actualStart = def->getStartBodyLine(),
        actualEnd = def->getEndBodyLine();
    if (readCodeFragment(def->getBodyDef()->absFilePath(), actualStart,
                         actualEnd, codeFragment)) {
      // printf("Adding code fragment '%s' ext='%s'\n",
      //    codeFragment.data(),m_impl->defFileExt.data());
      ParserInterface *pIntf =
          Doxygen::parserManager->getParser(def->getDefFileExtension());
      pIntf->resetCodeParserState();
      // printf("Read:\n`%s'\n\n",codeFragment.data());
      MemberDef *thisMd = 0;
      if (def->definitionType() == Definition::TypeMember)
        thisMd = (MemberDef *)def;
...
static void definedAtLine(int line, QCString fileName, FTextStream &t) {
  ...
  if (lineMarkerPos != -1 && fileMarkerPos != -1) // should always pass this.
  {
    ...
    {
      t << refText.left(lineMarkerPos) << line
        << refText.mid(lineMarkerPos + 2, fileMarkerPos - lineMarkerPos - 2)
        << fileName << refText.right(refText.length() - fileMarkerPos - 2);
    } else // file marker before line marker
    {
      t << refText.left(fileMarkerPos) << fileName
        << refText.mid(fileMarkerPos + 2, lineMarkerPos - fileMarkerPos - 2)
        << line << refText.right(refText.length() - lineMarkerPos - 2);
    }
  } else {
    err("translation error: invalid markers in "
        "trDefinedAtLineInSourceFile()\n");
  }
}

static void generateDocbookForMember(MemberDef *md, FTextStream &t,
                                     Definition *def, bool detailed = 0) {
  ...
  if (detailed == 0) {
    ...
    if (md->memberType() == MemberType_Enumeration) {
      ...
      t << "_1" << md->anchor() << "\">" << convertToDocBook(md->name())
        << "</link>";
      if (enumFields != 0) {
        ...
        for (emli.toFirst(); (emd = emli.current()); ++emli) {
          ...
          t << "<link linkend=\"_" << memberOutputFileBase(emd) << "_1"
            << emd->anchor() << "\">";
      ...
      if (!md->initializer().isEmpty() && md->initializer().length() < 2000) {
        t << " ";
        linkifyText(TextGeneratorDocbookImpl(t), def, md->getBodyDef(), md,
                    md->initializer());
      }
      if (md->briefDescription()) {
        t << "<para><emphasis>";
      ...
      if (md->getClassDef()) {
        t << "                        <para>"
          << convertToDocBook(md->declaration());
        if (md->briefDescription()) {
          t << "<para><emphasis>";
          writeDocbookString(t, md->briefDescription());
          t << "</emphasis></para>";
        ...
        t << " <link linkend=\"_";
        if (md->getGroupDef() &&
            def->definitionType() == Definition::TypeGroup) {
          t << md->getGroupDef()->getOutputFileBase();
        } else {
          t << memberOutputFileBase(md);
        }
    ...
    if (md->memberType() == MemberType_Enumeration) {
      ...
      t << "_1" << md->anchor() << "\">" << endl;
      t << "               <title>" << memType << " "
        << convertToDocBook(md->name()) << " "
        << "</title>" << endl;
      t << "               ";
      writeDocbookDocBlock(t, md->docFile(), md->docLine(), md->getOuterScope(),
                           md, md->documentation());
      t << endl;
      if (enumFields != 0) {
        MemberListIterator emli(*enumFields);
        ...
        t << "               <formalpara>" << endl;
        t << "                    <title>"
          << theTranslator->trEnumerationValues() << ":</title>" << endl;
        t << "                    <variablelist>" << endl;
        for (emli.toFirst(); (emd = emli.current()); ++emli) {
          t << "                        <varlistentry xml:id=\"_";
          t << memberOutputFileBase(emd) << "_1" << emd->anchor() << "\">"
            << endl;
          t << "                            <term>";
          writeDocbookString(t, emd->name());
          t << "</term>" << endl;
      ...
      t << "_1" << md->anchor() << "\">" << endl;
      t << "                <title>" << convertToDocBook(md->definition())
        << "</title>";
      if (Config_getBool(REPEAT_BRIEF)) {
        t << " <emphasis>";
        writeDocbookString(t, md->briefDescription());
      ...
      t << "_1" << md->anchor() << "\">" << endl;
      t << "                <title>" << convertToDocBook(md->definition())
        << " " << convertToDocBook(md->argsString()) << "</title>";
      ...
      if (Config_getBool(REFERENCED_BY_RELATION)) {
        generateSourceRefList(t, md->name(), theTranslator->trReferencedBy(),
                              md->getReferencedByMembers(), md);
      }
      if (Config_getBool(REFERENCES_RELATION)) {
        generateSourceRefList(t, md->name(), theTranslator->trReferences(),
                              md->getReferencesMembers(), md);
      }
      ...
      if (md->documentation()) {
        ...
        t << "                <title>" << convertToDocBook(md->definition())
          << "</title>";
        t << "                ";
        writeDocbookDocBlock(t, md->docFile(), md->docLine(),
                             md->getOuterScope(), md, md->documentation());
        t << endl;
        t << "            </section>" << endl;
      }
    } else if (md->memberType() == MemberType_Variable) {
...
static void generateDocbookSection(Definition *d, FTextStream &t,
                                   MemberList *ml, const char *,
                                   bool detailed = 0, const char *header = 0,
  ...
  case MemberListType_pubMethods:
    d->getLanguage() == SrcLangExt_ObjC
        ? title = theTranslator->trInstanceMethods()
        : title = theTranslator->trPublicMembers();
    switch (d->getLanguage()) {
    case SrcLangExt_ObjC:
      desctitle = theTranslator->trMethodDocumentation();
      break;
    case SrcLangExt_Fortran:
      desctitle = theTranslator->trMemberFunctionDocumentationFortran();
      break;
    default:
      desctitle = theTranslator->trMemberFunctionDocumentation();
      break;
    };
    break;
  case MemberListType_priMethods:
    title = theTranslator->trPrivateMembers();
    desctitle = "";
    break;
  ...
  case MemberListType_related:
    title = theTranslator->trRelatedFunctions();
    desctitle = theTranslator->trRelatedFunctionDocumentation();
    subtitle = theTranslator->trRelatedSubscript();
    break;
  default:
    title = "";
    desctitle = "";
  ...
  if (detailed) {
    ...
    if (!QCString(header).isEmpty()) {
      t << "        <section>" << endl;
      t << "            <title>" << convertToDocBook(header) << "</title>"
        << endl;
    } else if (desctitle) {
      t << "        <section>" << endl;
  ...
  for (mli.toFirst(); (md = mli.current()); ++mli) {
    ...
    if (d->definitionType() != Definition::TypeFile ||
        md->getNamespaceDef() == 0) {
      if (detailed && md->documentation().isEmpty() &&
  ...
  if (cl) {
    ...
    for (cli.toFirst(); (cd = cli.current()); ++cli) {
      if (!cd->isHidden() && cd->name().find('@') == -1) {
        ...
        t << "                    <listitem>" << endl;
        t << "                        <para>"
          << "struct <link linkend=\"_" << classOutputFileBase(cd) << "\">"
          << convertToDocBook(cd->name()) << "</link>";
        t << "</para>" << endl;
  ...
  if (nl) {
    ...
    for (nli.toFirst(); (nd = nli.current()); ++nli) {
          ...
          nd->name().find('@') == -1) // skip anonymouse scopes
      {
        ...
        t << "                    <listitem>" << endl;
        t << "                        <para>"
          << "struct <link linkend=\"_" << nd->getOutputFileBase() << "\">"
          << convertToDocBook(nd->name()) << "</link>";
        t << "</para>" << endl;
        t << "                    </listitem>" << endl;
        t << "                </itemizedlist>" << endl;
        t << "            </para>" << endl;
...
static void writeInnerFiles(const FileList *fl, FTextStream &t) {
  if (fl) {
    ...
    for (fli.toFirst(); (fd = fli.current()); ++fli) {
      ...
      t << "                    <listitem>" << endl;
      t << "                        <para>"
        << "file <link linkend=\"_" << fd->getOutputFileBase() << "\">"
        << convertToDocBook(fd->name()) << "</link>";
      t << "</para>" << endl;
      t << "                    </listitem>" << endl;
      t << "                </itemizedlist>" << endl;
      t << "            </para>" << endl;
...
static void writeInnerPages(const PageSDict *pl, FTextStream &t) {
  if (pl) {
    ...
    for (pli.toFirst(); (pd = pli.current()); ++pli) {
      t << "<xi:include href=\"" << pd->getOutputFileBase()
...
static void writeInnerGroups(const GroupList *gl, FTextStream &t) {
  if (gl) {
    ...
    if (gli.toFirst()) {
      t << "    <simplesect>" << endl;
      t << "        <title>" << theTranslator->trModules() << "</title>"
        << endl;
      t << "    </simplesect>" << endl;
      t << "    <para>" << endl;
      t << "        <itemizedlist>" << endl;
    ...
    for (gli.toFirst(); (sgd = gli.current()); ++gli) {
      t << "            <listitem><para><link linkend=\"_"
        << sgd->getOutputFileBase() << "\">"
        << convertToDocBook(sgd->groupTitle()) << "</link></para></listitem>"
        << endl;
    }
...
static void writeInnerDirs(const DirList *dl, FTextStream &t) {
  if (dl) {
    ...
    for (subdirs.toFirst(); (subdir = subdirs.current()); ++subdirs) {
      ...
      t << "                    <listitem>" << endl;
      t << "                        <para>"
        << "dir <link linkend=\"_" << subdir->getOutputFileBase() << "\">"
        << convertToDocBook(subdir->displayName()) << "</link>";
      t << "</para>" << endl;
      t << "                    </listitem>" << endl;
      t << "                </itemizedlist>" << endl;
      t << "            </para>" << endl;
...
static void generateDocbookForClass(ClassDef *cd, FTextStream &ti) {
  ...
  ti << "        <xi:include href=\"" << fileDocbook
  ...
  if (cd->briefDescription()) {
    t << "    <para>" << endl;
    writeDocbookDocBlock(t, cd->briefFile(), cd->briefLine(), cd, 0,
                         cd->briefDescription());
    t << "    </para>" << endl;
  }
  ...
  if (Config_getBool(HAVE_DOT) &&
    ...
    t << "<para>Inheritance diagram for " << convertToDocBook(cd->name())
      << "</para>" << endl;
    DotClassGraph inheritanceGraph(cd, DotNode::Inheritance);
    inheritanceGraph.writeGraph(t, GOF_BITMAP, EOF_DocBook,
                                Config_getString(DOCBOOK_OUTPUT), fileName,
                                relPath, TRUE, FALSE);
  }

  if (Config_getBool(HAVE_DOT) && Config_getBool(COLLABORATION_GRAPH)) {
    t << "<para>Collaboration diagram for " << convertToDocBook(cd->name())
      << "</para>" << endl;
    DotClassGraph collaborationGraph(cd, DotNode::Collaboration);
    collaborationGraph.writeGraph(t, GOF_BITMAP, EOF_DocBook,
                                  Config_getString(DOCBOOK_OUTPUT), fileName,
                                  relPath, TRUE, FALSE);
  }
  ...
  if (cd->getMemberGroupSDict()) {
    ...
    for (; (mg = mgli.current()); ++mgli) {
      generateDocbookSection(cd, t, mg->members(), "user-defined", 0,
                             mg->header(), mg->documentation());
    }
  }
  ...
  for (mli.toFirst(); (ml = mli.current()); ++mli) {
    if ((ml->listType() & MemberListType_detailedLists) == 0) {
      generateDocbookSection(cd, t, ml,
                             g_docbookSectionMapper.find(ml->listType()));
    }
  }

  if ((Config_getBool(REPEAT_BRIEF) && cd->briefDescription()) ||
      cd->documentation()) {
    t << "        <simplesect>" << endl;
    t << "            <title>" << theTranslator->trDetailedDescription()
      << "</title>" << endl;
    ...
    if (cd->documentation()) {
      writeDocbookDocBlock(t, cd->docFile(), cd->docLine(), cd, 0,
                           cd->documentation());
    }
    t << "        </simplesect>" << endl;
  }
  ...
  if (cd->getMemberGroupSDict()) {
    ...
    MemberGroup *mg;
    for (; (mg = mgli.current()); ++mgli) {
      generateDocbookSection(cd, t, mg->members(), "user-defined", 1,
                             mg->header(), mg->documentation());
    }
  }
  ...
  for (mli.toFirst(); (ml = mli.current()); ++mli) {
    if ((ml->listType() & MemberListType_detailedLists) == 0) {
      generateDocbookSection(cd, t, ml,
                             g_docbookSectionMapper.find(ml->listType()), 1);
    }
  }

  /*// TODO: Handling of Inheritance and Colloboration graph for Docbook to be
    implemented DotClassGraph inheritanceGraph(cd,DotNode::Inheritance); if
    (!inheritanceGraph.isTrivial())
    {
  ...
  t << "                <para>" << cd->generatedFromFiles() << "</para>"
    << endl;
  t << "                <para><itemizedlist><listitem><para>"
    << stripPath(cd->getDefFileName())
...
static void generateDocbookForNamespace(NamespaceDef *nd, FTextStream &ti) {
  ...
  if (nd->getMemberGroupSDict()) {
    ...
    MemberGroup *mg;
    for (; (mg = mgli.current()); ++mgli) {
      generateDocbookSection(nd, t, mg->members(), "user-defined", 0,
                             mg->header(), mg->documentation());
    }
  }
  ...
  for (mli.toFirst(); (ml = mli.current()); ++mli) {
    if ((ml->listType() & MemberListType_declarationLists) != 0) {
      generateDocbookSection(nd, t, ml,
                             g_docbookSectionMapper.find(ml->listType()));
    }
  }

  if ((Config_getBool(REPEAT_BRIEF) && nd->briefDescription()) ||
      nd->documentation()) {
    t << "        <simplesect>" << endl;
    t << "            <title>" << theTranslator->trDetailedDescription()
      << "</title>" << endl;
    ...
    if (Config_getBool(REPEAT_BRIEF)) {
      if (nd->briefDescription()) {
        ...
        // t << "        <title>" << theTranslator->trBriefDescription() <<
        // "</title>" << endl;
        writeDocbookDocBlock(t, nd->briefFile(), nd->briefLine(), nd, 0,
                             nd->briefDescription());
        t << "    </para>" << endl;
      }
    }
    ...
    if (nd->documentation()) {
      writeDocbookDocBlock(t, nd->docFile(), nd->docLine(), nd, 0,
                           nd->documentation());
    }
    t << "        </simplesect>" << endl;
  }
  ...
  if (nd->getMemberGroupSDict()) {
    ...
    MemberGroup *mg;
    for (; (mg = mgli.current()); ++mgli) {
      generateDocbookSection(nd, t, mg->members(), "user-defined", 1,
                             mg->header(), mg->documentation());
    }
  }
  ...
  // t << "                <para>" <<
  // theTranslator->trGeneratedFromFiles(ClassDef::Struct, FALSE) << "</para>"
  // << endl; t << "                <para><itemizedlist><listitem><para>" <<
  // stripPath(nd->getDefFileName()) <<
  // "</para></listitem></itemizedlist></para>" << endl;
  t << "</section>" << endl;
}
...
static void generateDocbookForFile(FileDef *fd, FTextStream &ti) {
  ...
  if (fd->briefDescription()) {
    t << "    <para>" << endl;
    writeDocbookDocBlock(t, fd->briefFile(), fd->briefLine(), fd, 0,
                         fd->briefDescription());
    t << "    </para>" << endl;
  }
  ...
  if (Config_getBool(HAVE_DOT)) {
    if (Config_getBool(INCLUDE_GRAPH)) {
      t << "<para>Include dependency diagram for "
        << convertToDocBook(fd->name()) << "</para>" << endl;
      DotInclDepGraph idepGraph(fd, FALSE);
      idepGraph.writeGraph(t, GOF_BITMAP, EOF_DocBook,
                           Config_getString(DOCBOOK_OUTPUT), fileName, relPath,
                           FALSE);
    }
    if (Config_getBool(INCLUDED_BY_GRAPH)) {
      t << "<para>Included by dependency diagram for "
        << convertToDocBook(fd->name()) << "</para>" << endl;
      DotInclDepGraph ibdepGraph(fd, TRUE);
      ibdepGraph.writeGraph(t, GOF_BITMAP, EOF_DocBook,
                            Config_getString(DOCBOOK_OUTPUT), fileName, relPath,
                            FALSE);
    }
  }
  ...
  if (fd->getMemberGroupSDict()) {
    ...
    MemberGroup *mg;
    for (; (mg = mgli.current()); ++mgli) {
      generateDocbookSection(fd, t, mg->members(), "user-defined", 0,
                             mg->header(), mg->documentation());
    }
  }
  ...
  for (mli.toFirst(); (ml = mli.current()); ++mli) {
    if ((ml->listType() & MemberListType_declarationLists) != 0) {
      generateDocbookSection(fd, t, ml,
                             g_docbookSectionMapper.find(ml->listType()));
    }
  }

  t << "    <simplesect>" << endl;
  t << "        <title>" << theTranslator->trDetailedDescription() << "</title>"
    << endl;
  if (Config_getBool(REPEAT_BRIEF)) {
    if (fd->briefDescription()) {
      writeDocbookDocBlock(t, fd->briefFile(), fd->briefLine(), fd, 0,
  ...
  writeDocbookDocBlock(t, fd->docFile(), fd->docLine(), fd, 0,
                       fd->documentation());
  if (Config_getBool(FULL_PATH_NAMES)) {
    t << "    <para>Definition in file " << fd->getDefFileName() << "</para>"
      << endl;
  } else {
    t << "    <para>Definition in file " << stripPath(fd->getDefFileName())
      << "</para>" << endl;
  }
  t << "    </simplesect>" << endl;
...
static void generateDocbookForGroup(GroupDef *gd, FTextStream &ti) {
  ...
  t << "    <title>" << convertToDocBook(gd->groupTitle()) << "</title>"
    << endl;
  if (gd->briefDescription()) {
    t << "    <para>" << endl;
    writeDocbookDocBlock(t, gd->briefFile(), gd->briefLine(), gd, 0,
                         gd->briefDescription());
    t << "    </para>" << endl;
  }

  if (Config_getBool(GROUP_GRAPHS) && Config_getBool(HAVE_DOT)) {
    t << "<para>Collaboration diagram for "
      << convertToDocBook(gd->groupTitle()) << "</para>" << endl;
  ...
  if (gd->getMemberGroupSDict()) {
    ...
    MemberGroup *mg;
    for (; (mg = mgli.current()); ++mgli) {
      generateDocbookSection(gd, t, mg->members(), "user-defined", 0,
                             mg->header(), mg->documentation());
    }
  }
  ...
  for (mli.toFirst(); (ml = mli.current()); ++mli) {
    if ((ml->listType() & MemberListType_declarationLists) != 0) {
      generateDocbookSection(gd, t, ml,
                             g_docbookSectionMapper.find(ml->listType()));
    }
  }
  ...
  if (gd->documentation()) {
    ...
    t << "            <title>" << theTranslator->trDetailedDescription()
      << "</title>" << endl;
    writeDocbookDocBlock(t, gd->docFile(), gd->docLine(), gd, 0,
                         gd->documentation());
    t << "        </simplesect>" << endl;
  }
...
static void generateDocbookForDir(DirDef *dd, FTextStream &ti) {
  ...
  if (dd->briefDescription()) {
    writeDocbookDocBlock(t, dd->briefFile(), dd->briefLine(), dd, 0,
                         dd->briefDescription());
  }
  if (Config_getBool(DIRECTORY_GRAPH) && Config_getBool(HAVE_DOT)) {
    t << "<para>Directory dependency diagram for "
      << convertToDocBook(dd->displayName()) << "</para>" << endl;
    DotDirDeps dirdepGraph(dd);
    dirdepGraph.writeGraph(t, GOF_BITMAP, EOF_DocBook,
                           Config_getString(DOCBOOK_OUTPUT), fileName, relPath,
                           FALSE);
  }
  ...
  writeDocbookDocBlock(t, dd->docFile(), dd->docLine(), dd, 0,
                       dd->documentation());
...
static void generateDocbookForPage(PageDef *pd, FTextStream &ti,
                                   bool isExample) {
  // + name
  // + title
  // + documentation
  ...
  if (si) {
    if (pageName == "mainpage")
      t << "    <title>" << convertToDocBook(theTranslator->trMainPage())
        << "</title>" << endl;
    else
...
void generateDocbook_v1() {
  ...
  t << "<?xml version='1.0' encoding='UTF-8' standalone='no'?>" << endl;
  ;
  t << "<book xmlns=\"http://docbook.org/ns/docbook\" version=\"5.0\" "
       "xmlns:xlink=\"http://www.w3.org/1999/xlink\">"
    << endl;
  t << "    <info>" << endl;
  ...
  if (gli.toFirst()) {
    t << "    <chapter>" << endl;
    t << "        <title>" << theTranslator->trModuleDocumentation()
      << "</title>" << endl;
  }
...

  {
    ...
    if (cli.toFirst()) {
      t << "    <chapter>" << endl;
      t << "        <title>" << theTranslator->trClassDocumentation()
        << "</title>" << endl;
    }
  ...
  if (showFiles) {
    ...
    if (fnli.toFirst()) {
      t << "    <chapter>" << endl;
      t << "        <title>" << theTranslator->trFileDocumentation()
        << "</title>" << endl;
    }
  ...
  if (Config_getBool(DIRECTORY_GRAPH) && Config_getBool(HAVE_DOT)) {
    ...
    if (sdi.toFirst()) {
      t << "    <chapter>" << endl;
      t << "        <title>" << theTranslator->trDirDocumentation()
        << "</title>" << endl;
    }
...

  {
    ...
    if (pdi.toFirst()) {
      t << "    <chapter>" << endl;
      t << "        <title>" << theTranslator->trExampleDocumentation()
        << "</title>" << endl;
    }
...
DocbookGenerator::DocbookGenerator() : OutputGenerator() {
  ...
  m_inLevel = -1;
  m_firstMember = FALSE;
  for (int i = 0; i < sizeof(m_inListItem) / sizeof(*m_inListItem); i++)
    m_inListItem[i] = FALSE;
  for (int i = 0; i < sizeof(m_inSimpleSect) / sizeof(*m_inSimpleSect); i++)
    m_inSimpleSect[i] = FALSE;
}

DocbookGenerator::~DocbookGenerator() { DB_GEN_C }
...
void DocbookGenerator::endIndexSection(IndexSections is) {
  ...
  case isMainPage:
    t << "</title>" << endl;
    t << "    <xi:include href=\"mainpage.xml\" "
  ...
  case isModuleIndex:
    ...
    break;
  case isDirIndex:
    // t << "<xi:include href=\"dirs.xml\"
    // xmlns:xi=\"http://www.w3.org/2001/XInclude\"/>"; t << "</chapter>" <<
    // endl;
    break;
  case isNamespaceIndex:
    // t << "<xi:include href=\"namespaces.xml\"
    // xmlns:xi=\"http://www.w3.org/2001/XInclude\"/>"; t << "</chapter>" <<
    // endl;
    break;
  case isClassHierarchyIndex:
    // t << "<xi:include href=\"hierarchy.xml\"
    // xmlns:xi=\"http://www.w3.org/2001/XInclude\"/>"; t << "</chapter>" <<
    // endl;
    break;
  case isCompoundIndex:
    ...
    break;
  case isFileIndex:
    // t << "<xi:include href=\"files.xml\"
    // xmlns:xi=\"http://www.w3.org/2001/XInclude\"/>"; t << "</chapter>" <<
    // endl;
    break;
  case isPageIndex:
    // t << "<xi:include href=\"pages.xml\"
    // xmlns:xi=\"http://www.w3.org/2001/XInclude\"/>"; t << "</chapter>" <<
    // endl;
    break;
  case isModuleDocumentation: {
    ...
    for (gli.toFirst(); (gd = gli.current()) && !found; ++gli) {
      if (!gd->isReference()) {
        t << "    <xi:include href=\"" << gd->getOutputFileBase()
          << ".xml\" xmlns:xi=\"http://www.w3.org/2001/XInclude\"/>" << endl;
        found = TRUE;
      }
    }
  ...
  case isDirDocumentation: {
    ...
    for (dli.toFirst(); (dd = dli.current()) && !found; ++dli) {
      if (dd->isLinkableInProject()) {
        t << "<    xi:include href=\"" << dd->getOutputFileBase()
          << ".xml\" xmlns:xi=\"http://www.w3.org/2001/XInclude\"/>" << endl;
        found = TRUE;
      }
    }
  ...
  case isNamespaceDocumentation: {
    ...
    for (nli.toFirst(); (nd = nli.current()) && !found; ++nli) {
      if (nd->isLinkableInProject()) {
        t << "<xi:include href=\"" << nd->getOutputFileBase()
          << ".xml\" xmlns:xi=\"http://www.w3.org/2001/XInclude\"/>" << endl;
        found = TRUE;
      }
    }
  ...
  case isClassDocumentation: {
    ...
    for (cli.toFirst(); (cd = cli.current()) && !found; ++cli) {
      if (cd->isLinkableInProject() && cd->templateMaster() == 0 &&
        ...
        t << "    <xi:include href=\"" << cd->getOutputFileBase()
          << ".xml\" xmlns:xi=\"http://www.w3.org/2001/XInclude\"/>" << endl;
        found = TRUE;
      }
    }
    ...
    for (fnli.toFirst(); (fn = fnli.current()); ++fnli) {
      ...
      for (; (fd = fni.current()); ++fni) {
        if (fd->isLinkableInProject()) {
          if (isFirst) {
            t << "    <xi:include href=\"" << fd->getOutputFileBase()
              << ".xml\" xmlns:xi=\"http://www.w3.org/2001/XInclude\"/>"
              << endl;
            if (sourceBrowser && m_prettyCode && fd->generateSourceFile()) {
              t << "    <xi:include href=\"" << fd->getSourceFileBase()
                << ".xml\" xmlns:xi=\"http://www.w3.org/2001/XInclude\"/>"
                << endl;
            }
            isFirst = FALSE;
...
void DocbookGenerator::writeDoc(DocNode *n, Definition *ctx, MemberDef *) {
  DB_GEN_C
  DocbookDocVisitor *visitor = new DocbookDocVisitor(t, *this);
  n->accept(visitor);
  delete visitor;
}
...
void DocbookGenerator::writeObjectLink(const char *ref, const char *f,
  ...
  if (anchor)
    if (f)
      t << "<link linkend=\"_" << stripPath(f) << "_1" << anchor << "\">";
    else
      t << "<link linkend=\"_" << anchor << "\">";
  else
...
void DocbookGenerator::endParameterList() { DB_GEN_C }
...
void DocbookGenerator::startMemberDoc(const char *clname, const char *memname,
  ...
  if (memTotal > 1) {
    t << "<computeroutput>[" << memCount << "/" << memTotal
      << "]</computeroutput>";
  }
  t << "</title>" << endl;
...
void DocbookGenerator::endMemberDocName() { DB_GEN_C }
...
void DocbookGenerator::startMemberGroup() { DB_GEN_C }
...
void DocbookGenerator::startLabels() { DB_GEN_C }
...
void DocbookGenerator::endLabels() { DB_GEN_C }
...
void DocbookGenerator::endParameterName(bool last, bool /*emptyList*/,
                                        bool closeBracket) {
  DB_GEN_C
  if (last) {
    if (closeBracket)
...
void DocbookGenerator::startMemberTemplateParams() { DB_GEN_C }
...
void DocbookGenerator::startSection(const char *lab, const char *,
...
void DocbookGenerator::startDescTable(const char *title) {
  ...
  if (title)
    t << "<title>" << convertToDocBook(title) << "</title>" << endl;
  t << "    <tgroup cols=\"" << ncols
    << "\" align=\"left\" colsep=\"1\" rowsep=\"1\">" << endl;
...
void DocbookGenerator::startDescTableTitle() { DB_GEN_C }

void DocbookGenerator::endDescTableTitle() { DB_GEN_C }
...
void DocbookGenerator::startGroupCollaboration() { DB_GEN_C }
void DocbookGenerator::endGroupCollaboration(const DotGroupCollaboration &g) {
  DB_GEN_C
  g.writeGraph(t, GOF_BITMAP, EOF_DocBook, Config_getString(DOCBOOK_OUTPUT),
               fileName, relPath, FALSE);
}
void DocbookGenerator::startDotGraph() { DB_GEN_C }
void DocbookGenerator::endDotGraph(const DotClassGraph &g) {
  DB_GEN_C
  g.writeGraph(t, GOF_BITMAP, EOF_DocBook, Config_getString(DOCBOOK_OUTPUT),
               fileName, relPath, TRUE, FALSE);
}
void DocbookGenerator::startInclDepGraph() { DB_GEN_C }
...
void DocbookGenerator::startCallGraph() { DB_GEN_C }
...
void DocbookGenerator::startDirDepGraph() { DB_GEN_C }
...
void DocbookGenerator::startMemberDocList() { DB_GEN_C }
...
void DocbookGenerator::endConstraintParam() { DB_GEN_C }
...
void DocbookGenerator::startConstraintDocs() { DB_GEN_C }
void DocbookGenerator::endConstraintDocs() { DB_GEN_C }
