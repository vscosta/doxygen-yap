...
public:
  ...
  Private()
      : tu(0), tokens(0), numTokens(0), cursors(0), ufs(0), sources(0),
        numFiles(0), fileMapping(257), detectedLang(Detected_Cpp) {
...
static void inclusionVisitor(CXFile includedFile,
                             ...
                             unsigned /*includeLen*/, CXClientData clientData) {
...
void ClangParser::start(const char *fileName,
  ...
  static QCString clangCompileDatabase =
      Config_getList(CLANG_COMPILATION_DATABASE_PATH);
  ...
  // (https://clang.llvm.org/docs/JSONCompilationDatabase.html) this only needs
  // to be loaded once, and could be refactored to a higher level function
  static std::unique_ptr<clang::tooling::CompilationDatabase> db =
      clang::tooling::CompilationDatabase::loadFromDirectory(
          clangCompileDatabase.data(), error);
  ...
  if (strcmp(clangCompileDatabase, "0") != 0) {
    if (db == nullptr) {
      ...
      err("%s using clang compilation database path of: \"%s\"\n",
          error.c_str(), clangCompileDatabase.data());
    } else {
      // check if the file we are parsing is in the DB
      command = db->getCompileCommands(fileName);
      if (!command.empty()) {
        // it's possible to have multiple entries for the same file, so use the
        // last entry
  ...
  char **argv = (char **)malloc(
      sizeof(char *) * (4 + Doxygen::inputPaths.count() + includePath.count() +
                        clangOptions.count() + clang_option_len));
  if (!command.empty()) {
    std::vector<std::string> options = command[command.size() - 1].CommandLine;
    // copy each compiler option used from the database. Skip the first which is
    // compiler exe.
    for (auto option = options.begin() + 1; option != options.end(); option++) {
      argv[argc++] = qstrdup(option->c_str());
    }
    // this extra addition to argv is accounted for as we are skipping the first
    // entry in
    ...
    // we use the source file to detected the language. Detection will fail if
    // you pass a bunch of .h files containing ObjC code, and no sources :-(
    SrcLangExt lang = getLanguageFromFileName(fileName);
    if (lang == SrcLangExt_ObjC ||
      ...
      if (p->detectedLang == ClangParser::Private::Detected_Cpp &&
          (fn.right(4).lower() == ".cpp" || fn.right(4).lower() == ".cxx" ||
           fn.right(3).lower() == ".cc" ||
           fn.right(2).lower() == ".c")) { // fall back to C/C++ once we see an
                                           // extension that indicates this
        p->detectedLang = ClangParser::Private::Detected_Cpp;
  ...
  p->tu = clang_parseTranslationUnit(
      p->index, 0, argv, argc, p->ufs, numUnsavedFiles,
  ...
  if (p->tu) {
    ...
    CXSourceLocation fileEnd =
        clang_getLocationForOffset(p->tu, f, p->ufs[0].Length);
...
void ClangParser::switchToFile(const char *fileName) {
  if (p->tu) {
    ...
    if (pIndex && *pIndex < p->numFiles) {
      ...
      CXSourceLocation fileBegin = clang_getLocationForOffset(p->tu, f, 0);
      CXSourceLocation fileEnd =
          clang_getLocationForOffset(p->tu, f, p->ufs[i].Length);
      CXSourceRange fileRange = clang_getRange(fileBegin, fileEnd);
...
QCString ClangParser::lookup(uint line, const char *symbol) {
  ...
  while (l <= line && p->curToken < p->numTokens && !found) {
    CXString tokenString =
        clang_getTokenSpelling(p->tu, p->tokens[p->curToken]);
    // if (l==line)
    //{
    //  printf("try to match symbol %s with token
    ...
    if (l == line &&
        strncmp(ts, symbol, tl) == 0) // found partial match at the correct line
    {
      int offset = tl;
      while (offset < sl) // symbol spans multiple tokens
      {
        ...
        char c;
        while (offset < sl && ((c = symbol[offset]) == ' ' || c == '\t' ||
                               c == '\r' || c == '\n')) {
          offset++;
        }
...
static void writeLineNumber(CodeOutputInterface &ol, FileDef *fd, uint line) {
  ...
  if (d && d->isLinkable()) {
    ...
    if (md && md->isLinkable()) // link to member
    {
      ...
      ol.writeLineNumber(md->getReference(), md->getOutputFileBase(),
                         md->anchor(), line);
    } else // link to compound
    {
      g_currentMemberDef = 0;
      ol.writeLineNumber(d->getReference(), d->getOutputFileBase(), d->anchor(),
                         line);
    }
  } else // no link
  {
...
static void writeMultiLineCodeLink(CodeOutputInterface &ol, FileDef *fd,
                                   uint &line, uint &column, Definition *d,
                                   const char *text) {
...
void ClangParser::linkInclude(CodeOutputInterface &ol, FileDef *fd, uint &line,
                              uint &column, const char *text) {
  ...
  if (ifd) {
    ol.writeCodeLink(ifd->getReference(), ifd->getOutputFileBase(), 0, text,
...
void ClangParser::linkMacro(CodeOutputInterface &ol, FileDef *fd, uint &line,
                            uint &column, const char *text) {
 ...
 if (d && d->isLinkable()) {
    if (g_insideBody && g_currentMemberDef &&
        ...
        (g_currentMemberDef != d ||
         g_currentLine < line)) // avoid self-reference
    {
...
static void detectFunctionBody(const char *s) {
  ...
  if (g_searchForBody &&
      (qstrcmp(s, ":") == 0 ||
       qstrcmp(s, "{") == 0)) // start of 'body' (: is for constructor)
  {
    ...
    case CXToken_Keyword:
      if (strcmp(s, "operator") == 0) {
        ...
        codifyLines(ol, fd, s, line, column,
                    cursorKind == CXCursor_PreprocessingDirective
                        ? "preprocessor"
                        : keywordToType(s));
      }
      break;
...
ClangParser::ClangParser() { p = new Private; }

ClangParser::~ClangParser() { delete p; }
...
QCString ClangParser::lookup(uint, const char *) { return ""; }

void ClangParser::writeSources(CodeOutputInterface &, FileDef *) {}

ClangParser::ClangParser() { p = NULL; }
