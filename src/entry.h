...
public:
  ...
  enum Sections {
    ...
    PREDDOC_SEC = 0x00200000, // Prolog
    COMPOUNDDOC_MASK = CLASSDOC_SEC | STRUCTDOC_SEC | UNIONDOC_SEC |
                       PREDDOC_SEC | INTERFACEDOC_SEC | EXCEPTIONDOC_SEC |
                       PROTOCOLDOC_SEC | CATEGORYDOC_SEC | SERVICEDOC_SEC |
                       SINGLETONDOC_SEC,
    ...
    PREDICATE_SEC = 0x1C000000, // Prolog
  };
  ...
  static const uint64 ForwardDecl =
      (1ULL << 15); // forward declarad template classes
  ...
  static const uint64 Readonly =
      (1ULL << 56);                         // on UNO IDL attribute or property
  ...
  void createNavigationIndex(EntryNav *rootNav, FileStorage *storage,
                             FileDef *fd);
...
public:
  ...
  QList<ListItemInfo>
      *sli; //!< special lists (test/todo/bug/deprecated/..) this entry is in
