...
NamespaceDef::NamespaceDef(const char *df, int dl, int dc, const char *name,
...
void NamespaceDef::writeTagFile(FTextStream &tagFile) {
  ...
  tagFile << "    <filename>" << convertToXML(getOutputFileBase())
          << Doxygen::htmlFileExtension << "</filename>" << endl;
  QCString idStr = id();
  if (!idStr.isEmpty()) {
...
void NamespaceDef::writeDetailedDescription(OutputList &ol,
  ...
  if (hasDetailedDescription()) {
    ...
    if (!briefDescription().isEmpty() && Config_getBool(REPEAT_BRIEF)) {
      ol.generateDoc(briefFile(), briefLine(), this, 0, briefDescription(),
                     FALSE, FALSE);
    }
    if (!briefDescription().isEmpty() && Config_getBool(REPEAT_BRIEF) &&
        !documentation().isEmpty()) {
...
void NamespaceDef::writeBriefDescription(OutputList &ol) {
  if (hasBriefDescription()) {
    DocRoot *rootNode =
        validatingParseDoc(briefFile(), briefLine(), this, 0,
...
void NamespaceDef::writeAuthorSection(OutputList &ol) {
  ...
  ol.endGroupHeader();
  ol.parseText(
      theTranslator->trGeneratedAutomatically(Config_getString(PROJECT_NAME)));
  ol.popGeneratorState();
}
...
void NamespaceDef::writeSummaryLinks(OutputList &ol) {
  ...
  for (eli.toFirst(); (lde = eli.current()); ++eli) {
    if ((lde->kind() == LayoutDocEntry::NamespaceClasses && classSDict &&
         classSDict->declVisible()) ||
        (lde->kind() == LayoutDocEntry::NamespaceNestedNamespaces &&
         namespaceSDict && namespaceSDict->declVisible())) {
      LayoutDocEntrySection *ls = (LayoutDocEntrySection *)lde;
      QCString label = lde->kind() == LayoutDocEntry::NamespaceClasses
                           ? "nested-classes"
                           : "namespaces";
      ...
      if (ml && ml->declVisible()) {
        ol.writeSummaryLink(0, MemberList::listTypeAsString(ml->listType()),
                            lmd->title(lang), first);
        first = FALSE;
      }
    }
  }
    ...
    for (mli.toFirst(); (md = mli.current()); ++mli) {
      if (md->getNamespaceDef() == this && md->isLinkable() &&
        ...
        if (md->isLinkableInProject()) {
          ...
          if (createSubDirs)
            ol.writeString("../../");
          ol.writeString(md->getOutputFileBase() + Doxygen::htmlFileExtension +
                         "#" + md->anchor());
          ol.writeString("\">");
...
void NamespaceDef::addUsingDirective(NamespaceDef *nd) {
  ...
  // printf("%p: NamespaceDef::addUsingDirective:
  // %s:%d\n",this,name().data(),usingDirList->count());
}

NamespaceSDict *NamespaceDef::getUsedNamespaces() const {
  // printf("%p: NamespaceDef::getUsedNamespace:
...
QCString NamespaceDef::getOutputFileBase() const { return fileName; }
...
QCString NamespaceDef::displayName(bool includeScope) const {
  ...
  // printf("NamespaceDef::displayName() %s->%s
    ...
    for (nli.toFirst(); (nd = nli.current()); ++nli) {
      ...
      if (nd->getUsedNamespaces()) {
        ...
        for (unli.toFirst(); (und = unli.current()); ++unli) {
          // printf("Adding namespace %s to the using list of
      ...
      if (nd->getUsedClasses()) {
        ...
        for (cli.toFirst(); (ucd = cli.current()); ++cli) {
          // printf("Adding class %s to the using list of
  ...
  for (ni.toFirst(); (nd = ni.current()); ++ni) {
    if (nd->isLinkable() && nd->hasDocumentation()) {
      ...
      if (!nd->briefDescription().isEmpty() &&
        ...
        ol.generateDoc(nd->briefFile(), nd->briefLine(), nd, 0,
                       nd->briefDescription(), FALSE, FALSE, 0, TRUE);
...
void NamespaceDef::writeMemberDeclarations(OutputList &ol, MemberListType lt,
                                           const QCString &title) {
...
void NamespaceDef::writeMemberDocumentation(OutputList &ol, MemberListType lt,
                                            const QCString &title) {
  MemberList *ml = getMemberList(lt);
  if (ml)
...
bool NamespaceDef::isLinkableInProject() const {
  ...
  return !name().isEmpty() && name().at(i) != '@' && // not anonymous
         (hasDocumentation() ||
          getLanguage() == SrcLangExt_CSharp) && // documented
...
QCString NamespaceDef::compoundTypeString() const {
  ...
  if (lang == SrcLangExt_Java) {
    ...
    if (isModule()) {
      ...
      err_full(getDefFileName(), getDefLine(), "Internal inconsistency: "
                                               "namespace in IDL not module, "
                                               "library or constant group");
    }
  }
  return "";
}
